...
- [x] **H3 · `lint --fix` corrupts source** — DONE. Added a `code_only()` line scanner that blanks string-literal and comment text; `redundant_to_string` detection now matches only genuine code, and `--fix` rewrites per-line only on lines whose *code* contains the pattern — never touching occurrences inside strings/comments. Unit tests added. Verified: a file with the pattern in a string literal is untouched; genuine redundant code is collapsed.
- [x] **H4 · `heal --fix` noise + DATA-SAFETY** — DONE. (1) Aggregated the per-event "Skipping {:?}" spam into a by-kind summary (`4290 × OrphanAnchor`). (2) CRITICAL data-safety bug found+fixed: the `MissingContract` fix wrote `source_path.with_extension("adoc")`, which **equals the source when the source is already a `.adoc`/doc** → it overwrote real docs with stubs (it destroyed this repo's docs/ + .agent/ when I wrongly ran it on the real tree; recovered from git). Now skips rather than writes when the target equals the source or already exists (store-first means a missing contract is regenerated by `gen`, not a disk stub). Verified by reproducing the exact clobber in a scratch dir — `architecture.adoc` byte-identical after `--fix`.
...
- **Phase C COMPLETE:** H3 ✅ (lint --fix data-safety), H4 ✅ (heal --fix noise + a CRITICAL data-safety clobber bug — `MissingContract` was overwriting real `.adoc` docs with stubs), M1 ✅ (watch --graph-sync now runs the real gen path), M3 ✅ (orphan output: "No actionable orphans (N expected metadata)"). 39 test suites green, 0 warnings.
- **CRITICAL process lesson (re-learned the hard way):** NEVER run a mutating aden command (`heal --fix`, `gen`, `lint --fix`) against the real repo during validation — only scratch dirs. `heal --fix` clobbered docs/ + .agent/ this session (recovered from git). The underlying bug is now fixed, but the discipline stands.
- **Remaining:** H7 `complete` LLM integration (XL feature, deferred), M4/M5 polish. Everything else closed.
