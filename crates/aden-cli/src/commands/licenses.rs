    ...
    match pkg.ecosystem {
        Ecosystem::Cargo => {
            ...
            if let Some(j) = http_json(&url).and_then(|j| j.get("version").cloned()) {
                return LicenseInfo {
                    license: j.get("license").and_then(|v| v.as_str()).map(str::to_string),
                    repository: j
                        .get("repository")
                        .and_then(|v| v.as_str())
                        .map(str::to_string),
        ...
        Ecosystem::Npm => {
            ...
            if let Some(j) = http_json(&url) {
                let license = j.get("license").and_then(|v| v.as_str()).map(str::to_string);
                let repository = j
                    .get("repository")
        ...
        Ecosystem::PyPI => {
            ...
            if let Some(info) = http_json(&url).and_then(|j| j.get("info").cloned()) {
                let license = info
                    ...
                    .filter(|s| !s.is_empty())
                    .or_else(|| {
                        info.get("license").and_then(|v| v.as_str()).filter(|s| {
...
fn render_markdown(
    ...
    md.push_str("All third-party packages are used in accordance with their respective licenses.\n");
    md.push_str("No proprietary code is bundled or modified without explicit permission.\n");
    md.push_str("\n---\nGenerated by Aden.\n");
    md
}

fn write_out(out: Option<&Path>, content: &str, what: &str) -> Result<(), Box<dyn std::error::Error>> {
