...
pub fn cmd_init(
    ...
    // ADR-003: keep the project tree correct by construction. The rebuildable
    // graph store/caches now live in the per-user data dir, but a legacy in-tree
    // store may appear (old aden, or a migration in progress) — ignore those,
    // while tracking the durable repo-coupled intent (overlays, constitution,
    // hooks, project.conf) so it travels with the repository.
    let aden_gitignore = "\
# Rebuildable artifacts — regenerated by 'aden gen', never commit (ADR-003).
store/
cache/
gen-cache.json
scan-cache.json
...
# Durable, repo-coupled intent — DO commit these.
!overlays/
!constitution.adoc
!hooks/
!project.conf
";
    std::fs::write(aden_dir.join(".gitignore"), aden_gitignore)?;
