...
/// Resolve a tree-sitter language by name through the pack's own registry.
///
/// Default builds enable neither dynamic loading nor downloads, so this registry
/// can return only grammars statically linked through `TSLP_LANGUAGES` and
/// `TSLP_LINK_MODE=static`. `grammars-download` deliberately enables the pack's
/// matched-version, platform-aware dynamic cache for long-tail languages. Keeping
/// cache discovery upstream avoids hard-coded package versions and Unix-only
/// `.so` assumptions in Aden.
#[cfg(feature = "generic")]
